...
import { decode, encode } from '../src/pem'
...
// Simple PEM message for testing
const _simplePem = '-----BEGIN SIMPLE MESSAGE-----\r\n'
  + 'SGVsbG8gV29ybGQh\r\n'
  + '-----END SIMPLE MESSAGE-----\r\n'
...
describe('pem', () => {
  ...
  it('should decode a simple PEM message correctly', () => {
    const msgs = decode(_simplePem)
    expect(msgs.length).toBe(1)
    expect(msgs[0].type).toBe('SIMPLE MESSAGE')

    // Convert Uint8Array to string for comparison
    // Use a different approach to convert the binary data to string
    const bodyText = Buffer.from(msgs[0].body).toString('utf-8')
    expect(bodyText).toBe('Hello World!')
  })

  it('should handle PEM messages with no headers', () => {
    const pem = '-----BEGIN NO HEADERS-----\r\n'
      + 'SGVsbG8gV29ybGQh\r\n'
      + '-----END NO HEADERS-----\r\n'

    const msgs = decode(pem)
    expect(msgs.length).toBe(1)
    expect(msgs[0].type).toBe('NO HEADERS')
    expect(msgs[0].procType).toBeNull()
    expect(msgs[0].contentDomain).toBeNull()
    expect(msgs[0].dekInfo).toBeNull()
    expect(msgs[0].headers.length).toBe(0)
  })

  it('should encode a PEM message with custom options', () => {
    const msg = decode(_simplePem)[0]

    // Encode with a different line length
    const encoded = encode(msg, { maxline: 40 })

    // Re-decode to verify it's still valid
    const reDecoded = decode(encoded)
    expect(reDecoded.length).toBe(1)
    expect(reDecoded[0].type).toBe('SIMPLE MESSAGE')

    // Verify the line length is respected (approximately)
    const lines = encoded.split('\r\n')
    // Skip first and last lines (BEGIN/END markers)
    for (let i = 1; i < lines.length - 2; i++) {
      expect(lines[i].length).toBeLessThanOrEqual(40)
    }
  })

  it('should throw an error for invalid PEM formatted message', () => {
    const invalidPem = '-----BEGIN INVALID-----\r\n'
      + 'Proc-Type: 4\r\n' // Missing the second subfield
      + 'SGVsbG8gV29ybGQh\r\n'
      + '-----END INVALID-----\r\n'

    expect(() => decode(invalidPem)).toThrow('Invalid PEM formatted message')
  })

  it('should throw an error for encrypted message without DEK-Info', () => {
    const invalidPem = '-----BEGIN ENCRYPTED MESSAGE-----\r\n'
      + 'Proc-Type: 4,ENCRYPTED\r\n' // Missing DEK-Info header
      + '\r\n'
      + 'SGVsbG8gV29ybGQh\r\n'
