  ...
  describe('rsa', () => {
    ...
    function isDeterministic(isPrng, isAsync, isPurejs) {
      ...
      if (UTIL.isNodejs) {
        ...
        if (isAsync && !isPurejs
          && typeof require('node:crypto').generateKeyPair === 'function') {
          return false
        }
        if (!isAsync && !isPurejs
          && typeof require('node:crypto').generateKeyPairSync === 'function') {
          return false
        }
      }
      else {
    ...
    (function () {
      ...
      algorithms.forEach((algorithm) => {
        prfAlgorithms.forEach((prfAlgorithm) => {
          it(`should PKCS#8 encrypt and decrypt private key with ${algorithm
          } encryption and ${prfAlgorithm} PRF`, () => {
            let privateKey = PKI.privateKeyFromPem(_pem.privateKey)
            const encryptedPem = PKI.encryptRsaPrivateKey(
              privateKey,
              'password',
      ...
      function createTests(params) {
        ...
        it(`should rsa sign using a ${keySize}-bit key and PSS padding`, () => {
          ...
          const pss = PSS.create(
            MD.sha1.create(),
            MGF.mgf1.create(MD.sha1.create()),
            20,
          )
        ...
        it(`should verify an rsa signature using a ${keySize}-bit key and PSS padding`, () => {
          ...
          ASSERT.equal(
            key.verify(md.digest().getBytes(), signature, pss),
            true,
          )
        })
